...
tau_u1 = dist.VectorField(coords, name='tau_u1', bases=disk.edge)
tau_u2 = dist.VectorField(coords, name='tau_u2', bases=disk.edge)
tau_w1 = dist.Field(name='tau_w1', bases=disk.edge)
tau_w2 = dist.Field(name='tau_w2', bases=disk.edge)
...
er = dist.VectorField(coords, name=f"er",bases=disk)
er['g'][1] = 1
grad_u = d3.grad(u) + er*lift(tau_u1) # First-order reduction
...
# Problem
problem = d3.EVP([u, w, p, tau_u1, tau_w1,tau_u2, tau_w2, tau_p], eigenvalue=s, namespace=locals())
problem.add_equation("div(u) + dz(w) = 0")
problem.add_equation("dt(u) + w0*dz(u) + grad(p) - (1/Re)*(div(grad_u)+dz(dz(u))) + lift(tau_u2) = 0")
problem.add_equation("dt(w) + w0*dz(w) + u@grad(w0) + dz(p) - (1/Re)*(lap(w)+dz(dz(w))) + lift(tau_w1)+lift(tau_w2)= 0")
problem.add_equation("u(r=0.5) = 0")
problem.add_equation("w(r=0.5) = 0")
problem.add_equation("u(r=1) = 0")
problem.add_equation("w(r=1) = 0")


# problem = d3.EVP([u, w, p, tau_u1, tau_w1, tau_p], eigenvalue=s, namespace=locals())
# problem.add_equation("div(u) + dz(w) = 0")
# problem.add_equation("dt(u) + w0*dz(u) + grad(p) - (1/Re)*(lap(u)+dz(dz(u))) + lift(tau_u1) = 0")
# problem.add_equation("dt(w) + w0*dz(w) + u@grad(w0) + dz(p) - (1/Re)*(lap(w)+dz(dz(w))) + lift(tau_w1) = 0")
# problem.add_equation("u(r=1) = 0")
# problem.add_equation("w(r=1) = 0")
...
for axi in ax.flatten():
    ...
    axi.set_axis_off()
fig.tight_layout()
fig.savefig("tc_eigenfunctions.png", dpi=200)